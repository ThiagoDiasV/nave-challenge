...
def test_create_new_user_without_email_info():
    with pytest.raises(ValueError):
        get_user_model().objects.create_user(email="", password="blablabla")


@pytest.mark.django_db
def test_create_new_user_successfully():
    get_user_model().objects.create_user(email="nave@nave.com", password="blablabla")
...
def test_create_new_superuser_successfully():
    get_user_model().objects.create_superuser(
        email="supernave@nave.com", password="blablabla"
    )
